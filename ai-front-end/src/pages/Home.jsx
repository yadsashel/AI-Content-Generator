...
import notebook from "../components/assets/Images/notebook.jpg";
import wordsc from "../components/assets/Images/wordsc.png";
import podcast from "../components/assets/Images/podcast.jpg";
...
export default function Home() {
  ...
  useEffect(() => {
    document.title = "Home - AI Content Generator"; // browser tab title
  }, []);
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        ...
        "template_vjalmuv",    // your Template ID
        form.current,
          ...
          >
            <motion.div variants={fadeUp} className="space-y-6">
              ...
              <p className="text-white/70 md:text-lg">
                Generate posts, threads, captions, ad copy & blog intros tailored
                for each platform — Instagram, X/Twitter, LinkedIn, TikTok, and
                more.
              </p>
              ...
              <div className="flex items-center gap-6 pt-4">
                <div className="flex -space-x-2">
                  {[notebook, wordsc, podcast].map((src, i) => (
                    <img
                      key={i}
                      src={src}
                      alt=""
                      className="h-9 w-9 rounded-full ring-2 ring-[#0B1020] object-cover"
                    />
                  ))}
                </div>
                <p className="text-sm text-white/60">Loved by creators & marketers</p>
              </div>
            </motion.div>
              ...
              <div className="relative rounded-3xl border border-white/10 bg-gradient-to-tr from-white/10 to-white/5 p-5 backdrop-blur-xl shadow-2xl">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <span className="text-xs text-white/60">
                    AI Draft • Instagram Caption
                  </span>
                </div>

                <div className="rounded-2xl bg-[#0B1020]/60 border border-white/10 p-4">
                  <p className="text-sm text-white/80">
                    “Launching our new summer line ☀️ Minimal, comfy, and made to
                    move. Drop a 🌊 if you’re ready!”
                  </p>
                </div>
      ...
      <section className="border-t border-white/10 bg-[#0C1226]">
        <div className="max-w-7xl mx-auto px-6 py-10">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6 opacity-80">
            {["Instagram", "X / Twitter", "LinkedIn", "YouTube", "TikTok", "Facebook"].map(
              (n) => (
                <span key={n} className="text-white/60 text-sm md:text-base">
                  {n}
                </span>
              )
            )}
        ...
        <div className="max-w-3xl mx-auto px-6 py-20">
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-3">
              ...
              <a
                href="/Register"
                className="inline-block rounded-xl bg-white px-5 py-3 font-semibold text-slate-900 shadow-lg shadow-white/10 hover:-translate-y-0.5 transition"
              >
                Generate draft
              </a>
            </div>
      ...
      <footer id="contact" className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-10">
          ...
          <form ref={form} onSubmit={sendEmail} className="grid gap-3">
            ...
            <button
              type="submit"
              className="justify-self-start rounded-xl bg-white px-5 py-3 font-semibold text-slate-900 shadow-lg shadow-white/10"
            >
              Send message
            </button>
          </form>
